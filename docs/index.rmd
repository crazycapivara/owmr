...
weather
#weather_icons <- sapply(df$weather, function(x) sprintf(icon_url, x$icon))
#weather_description <- sapply(df$weather, function(x) x$description)
datetimes <- df$dt %>% as.POSIXct(origin = "1970-01-01")

leaflet(data = df) %>% addProviderTiles("CartoDB.DarkMatter") %>%
  addMarkers(~coord.lon, ~coord.lat, popup = paste0(
    "<b>DT_</b>", datetimes, "<br>",
    "<b>NAME_</b>", df$name, "</br>",
    "<b>TEMP_</b>", df$main.temp, " °C</br>",
    "<b>HUMIDITY_</b>", df$main.humidity, " % </br>",
